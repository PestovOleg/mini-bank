...
func (c *AppConfig) GetAllConfig() []util.LogPathCfg {
	a := make([]util.LogPathCfg, 0, len(c.LoggerCfgs))
	for _, loggerCfg := range c.LoggerCfgs {
		a = append(a, util.NewLogPathCfg(loggerCfg.Encoding, loggerCfg.Output, loggerCfg.Level))
	}
